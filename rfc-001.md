...
In RFC XXX we proposed an easy & painless way to send SNT via Status so our marketing team can welcome new users with tokens during launch. It consisted of a flow to fix the issue of the unknown recipient wallet account when using the send and receive commands.
# Proposal
...
The specification is updated to introduce the 3 `content-types` and their required keys in `content`:

- `command/send`, used after the transaction has been sent to notify the recipient
  - `chain-id` 1 digit, the ID of the chain so we can quickly check if the message refers to a transaction on the same chain the recipient is using
  - `account` address of the recipient wallet account, not strictly necessary, but makes it easier for the recipient to check the transaction since they are stored per account
  - `transaction-hash` hash of the transaction

- `command/send-request`, used to show an intent to send funds, expecting a request in response with the recipient address
  - `chain-id` 1 digit, the ID of the chain
  - `asset` string symbol
  - `amount` bigint

- `command/request`, used to request funds or reply to a `command/send-request` message
  - `chain-id` 1 digit, the ID of the chain
  - `asset` string symbol
  - `amount` bigint
  - `account` address of the recipient wallet account


Later on, we MAY replace: 
- `command/send` by a regular message using a `transaction-hash-uri` https://notes.status.im/Mc8cH-HdS8qCIVd7fqetaA#
- `command/request` by a regular message using EIP-681 as text.
- `command/send-request` by a regular message using a modified version of EIP-681 with unknown recipient that can be turned in to a valid EIP-681 URI using string interpolation.

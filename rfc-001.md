...
When sending a transaction through the `send` command, `status-go` will hold the message until the transaction is confirmed and only send it then, so that `command/send` refers only to transactions that have been confirmed. On the receiving end, `status-go` will check the transaction and fill the message fields based on the transaction parameters before sending it to `status-react`.
